...
with tab1:
    st.subheader("🔔 Hutang yang Perlu Dibayar")

    target_date = st.date_input(
        "📅 Tampilkan hutang yang jatuh tempo sampai tanggal:",
        value=datetime.now().date(),
        help="Akan menampilkan semua hutang yang due date-nya pada atau sebelum tanggal ini."
    )

    df_jatuh_tempo = new_database.get_tagihan_jatuh_tempo("hutang", target_date)

    if df_jatuh_tempo.empty:
        st.success(f"✅ Tidak ada hutang yang jatuh tempo sampai {target_date.strftime('%d %b %Y')}.")
    else:
        ...
        with col2:
            st.metric("Total Sisa Hutang", f"Rp {total_sisa:,.0f}".replace(",", "."))
        ...
        st.download_button(
            ...
            file_name=f"Hutang_Jatuh_Tempo_{target_date.strftime('%d-%m-%Y')}.xlsx",
...
with tab2:
    ...
    with st.expander("ℹ️ Cara input pelunasan"):
        ...
        2. Pilih nota pembelian yang ingin dibayar
        3. Masukkan detail pembayaran
        """)
    
    suppliers = new_database.get_all_data_supplier(["id", "nama"])
    cust_opts = {"-- Pilih Supplier --": None}
    if not suppliers.empty:
        cust_opts.update(dict(zip(suppliers['nama'], suppliers['id'])))
    
    sel_cust = st.selectbox("Nama Supplier", options=list(cust_opts.keys()))
    
    if sel_cust != "-- Pilih Supplier --":
        id_cust = cust_opts[sel_cust]
        
        available_hutang = new_database.get_outstanding_invoices("hutang", id_cust)
        
        if available_hutang.empty:
            st.info("✅ Tidak ada hutang yang belum lunas untuk supplier ini.")
        else:
            st.markdown("### 📋 Daftar Nota Belum Lunas:")

            available_hutang['display'] = available_hutang.apply(
            ...
            # Dictionary mapping: display text -> id hutang
            inv_opts = dict(zip(available_hutang['display'], available_hutang['id']))
            ...
            # id_hutang yang dipilih (ini yang akan disimpan ke database)
            id_hutang_selected = inv_opts[sel_inv]
            
            # Ambil data detail nota yang dipilih
            selected_invoice = available_hutang[available_hutang['id'] == id_hutang_selected].iloc[0]
            ...
            with st.form("form_bayar_hutang"):
                ...
                if submit_bayar and not cancel_bayar:
                    if jumlah_bayar <= 0:
                        ...
                        success, msg = new_database.insert_pembayaran_hutang(
                            id_hutang=id_hutang_selected,
...
with tab3:
    ...
    df_hist = new_database.get_history_pembayaran("hutang", start_date, end_date)
    ...
    if not df_hist.empty:
        ...
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            df_hist.drop(columns=['id'], errors='ignore').to_excel(writer, index=False, sheet_name='Hutang')
        
        tanggal_download = datetime.now().strftime("%d-%m-%Y")

        st.download_button(
            label="📥 Download Riwayat Hutang (Excel)",
            data=output.getvalue(),
            file_name=f"Riwayat Pembayaran Hutang_{tanggal_download}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True
        )
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "no_nota_tagihan": st.column_config.TextColumn("No. Faktur Pembelian"),
                "partner": st.column_config.TextColumn("Supplier"),
        ...
        if not to_delete.empty:
            ...
            if st.button("🗑️ Konfirmasi Pembatalan"):
                ...
                for idx, row in to_delete.iterrows():
                    res, _ = new_database.delete_pembayaran("hutang", row['id'])
